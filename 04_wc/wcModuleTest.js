...
var wc = require('./wcModule.js').wc;
var wc1 = require('./version2flag.js').wc1;



// test ["count the lines in a file"] = function(){
// 	assert.deepEqual(2,wc.countLinesinFile("one.txt"));
// 	assert.deepEqual(0,wc.countLinesinFile("two.txt"));
// };


test ["count the bytes in a string"] = function(){
	assert.deepEqual(11,wc.countBytes("hi prashant"));
	assert.deepEqual(20,wc.countBytes("hi prashant tripathi"));
};

test ["count the lines in a string"] = function(){
	assert.deepEqual(1,wc.countLines("hi \n prashant"));
	assert.deepEqual(2,wc.countLines("hi  \n prashant \n tripathi"));
};

test ["count the words in a string"] = function(){
	assert.deepEqual(3,wc.countWords("hi prashant\n ram"));
	assert.deepEqual(3,wc.countWords("hi prashant tripathi"));
};

// test ["check text is available or not"] = function(){
// 	assert.ok(false==wc.checkText(""));
// 	assert.ok(true==wc.checkText("this is baby"));
// };
